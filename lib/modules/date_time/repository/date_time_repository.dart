...
Future<Either<Failure, AvailableAndNotAvailableResponse>> getAvailabilityStatus() async {
    if (await networkInfo.isConnected) {
      //its connected to internet , its safe to call API
      try {
        late final dynamic response;
        ...
        if (response.statusCode == 200) {
          //success return data
          return Right(AvailableAndNotAvailableResponse.fromJson(response.data));
        } else {
          //failure -- return business error
          return Left(
              Failure(ApiInternalStatus.failure, ResponseMessage.unKnown));
